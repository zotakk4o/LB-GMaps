...
class LB_GMaps_Database_Handler {
	...
	public function create_tables() {
		...
		$maps_sql = "CREATE TABLE IF NOT EXISTS {$this->get_maps_table_name()} (
			...
			street_view_control VARCHAR(50) NULL,
			fullscreen_control VARCHAR(50) NULL,
			map_type_control VARCHAR(50) NULL,
			map_types VARCHAR(50) NULL,
			width VARCHAR(50) NOT NULL DEFAULT \"60%\",
			height VARCHAR(50) NOT NULL DEFAULT \"800px\",
			styles text NULL,
			directions VARCHAR(50) NULL,
			dir_searching_field VARCHAR(50) NULL,
			dir_means_of_transport VARCHAR(50) NULL,
			dir_route_markers VARCHAR(50) NULL,
			dir_route_infowindow VARCHAR(50) NULL,
			dir_waypoints_markers VARCHAR(50) NULL,
			PRIMARY KEY (post_id)
		) $charset_collate;";
